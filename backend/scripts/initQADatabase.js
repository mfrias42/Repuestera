...
async function initQADatabase() {
  ...
  try {
    // Configuración para QA en Azure MySQL
    connection = await mysql.createConnection({
      host: process.env.DB_HOST || 'repuestera-mfrias-qa-server.mysql.database.azure.com',
      port: process.env.DB_PORT || 3306,
      user: process.env.DB_USER || 'repuestera_admin',
      password: process.env.DB_PASSWORD || 'Repuestera2024!',
      database: process.env.DB_NAME || 'repuestera_qa_db',
    ssl: {
    ...
    console.log('🔗 Conectado a Azure MySQL Database QA');
    ...
    if (productos[0].count === 0) {
      ...
      console.log('🔧 Productos de ejemplo insertados en QA');
    }
